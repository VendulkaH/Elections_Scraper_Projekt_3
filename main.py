...
import requests  # pro stahování dat z internetu
import sys       # pro práci s argumenty z příkazové řádky
import csv       # pro zápis dat do CSV souboru
from bs4 import BeautifulSoup  # pro analýzu a parsování HTML

# Funkce pro kontrolu vstupních argumentů skriptu
def zkontroluj_argumenty():
    ...
    # Ověření, že URL funguje a je dostupná
    try:
...
# Funkce pro stažení a zpracování HTML obsahu stránky
def nacti_html(url):
    try:
...
# Získá seznam obcí z úvodní stránky
...
# Získá výsledky voleb pro jednu obec
def ziskej_vysledky(obec_url):
    ...
    try:
        ...
        for radek in radky:
            ...
            # Dvě možné struktury tabulky: dle přítomnosti ID a počtu sloupců
    ...
    for tabulka in tabulky:
        for radek in tabulka.find_all("tr"):
            ...
            if len(bunky) >= 3:
                ...
                if posledni_bunka.find("a"):  # kontrola, zda je v poslední buňce odkaz (odlišení datového řádku)
...
# Získá celkové výsledky z více okrsků v jedné obci
def ziskej_soucet_vysledku_z_okrsku(obec_url):
    ...
    # Převod čísel zpět na string kvůli exportu do CSV
...
# Zapíše výsledky všech obcí do CSV souboru
...
# Spuštění hlavní funkce při zavolání skriptu
